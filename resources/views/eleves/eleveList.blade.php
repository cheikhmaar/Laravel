                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                            ...
                            @foreach($eleves as $eleve)
                                ...
                                <tr>
                                    <td scope="row"> {{ $eleve->id }} </a></td>
                                    <td>         {{ $eleve->nom  }} </a> </td>
                                    <td> {{ $eleve->prenom  }}</a></td>
                                    <td> {{ $eleve->classe->libelleClasse  }}</a> </td>
                                    <td align="center"><a href="/eleveList/{{ $eleve->id}}/eleveEdit" class="btn btn-primary" style="width: 100px;" my-3>Update</a></td>
                                    <td align="center">
                                        <form action="/eleveList/{{ $eleve->id }}"
                                              method="POST"
                                              style="display:inline"
                                              onsubmit="return confirm('Etes vous sure de vouloir supprimer {{$eleve->nom}} {{$eleve->prenom}} de la liste des eleveurs')">
                                            @csrf
                                            @method('DELETE')
